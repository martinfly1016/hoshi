...
当前暂定固定主计算库为 `tyme4ts@1.4.6`。

- 许可证：MIT，可用于商业产品，但需要保留版权和许可证声明。
- 使用范围：年柱、月柱、日柱、时柱、藏干、十神、基础五行结构。
- 产品规则层仍由自有 adapter 显式处理：标准时间 / 真太阳时、晚子时不换日 / 子初换日、时辰不详默认午时。
- 回归方式：继续使用 `calculation-lab.html` 和 fixtures 对照外部工具结果，若新增样例发现重大偏差，再评估替换或双跑策略。
...
- 真太阳时换算必须先于时辰和晚子时规则判断。例如测测 App 中，北京市东城区输入 `1988-02-15 23:00` 后显示真太阳时 `22:31`，实际按亥时盘处理，不触发子时换日。
...
- 测测 App 截图对照显示：必须选择出生地，按真太阳时计算，并在 23:01 明确标记「子时换日」。
...
- 与测测 App 继续比对时，必须先确认出生地；测测结果不能直接当作标准时间结果使用。
