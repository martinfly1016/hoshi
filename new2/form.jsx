...
function normalizeDisplayTerms(text) {
  return localizeReadingTerm(text);
}
...
function UserTagIndex({ tags, onNavigate }) {
  ...
  const activeTag = tags[activeTagIndex] || tags[0];
  ...
  return (
    <section className="user-tag-index">
      ...
      <h2>重要な読みをタグで確認</h2>
      <p>気になる要点を選ぶと、同じ場所で概要を切り替えて確認できます。さらに詳しく読む場合は「詳細説明」から深い鑑定へ進めます。</p>
      ...
      <article id="tag-detail-active" className="user-tag-detail is-active user-tag-reader">
        <div className={`user-tag-detail-icon tag-${activeTag.kind}`} aria-hidden="true">{userTagIcon(activeTag)}</div>
        <div className="user-tag-detail-body">
          <div className="user-tag-detail-head">
            <span className={`user-tag tag-${activeTag.kind}`}>
              <small>{activeTag.label}</small>
              <strong>{activeTag.value}</strong>
            </span>
          </div>
          <p>{activeTag.detail}</p>
          <em>{activeTag.evidence}</em>
        </div>
        <button className="user-tag-detail-action" onClick={() => onNavigate(activeTag)}>詳細説明</button>
      </article>
    </section>
  );
}
...
function BasicInfoPanel({ name, calculation, profile }) {
  ...
  return (
    <section id="basic-info-panel" className="basic-info-panel result-wide" aria-label="命式の基本情報">
...
function ResultOverviewPanel({ name, calculation, readingTags, onShowInsight, onShowFortune }) {
  const currentAnnual = currentAnnualFortune(calculation);
  const currentDecade = currentDecadeFortune(calculation.luckCycles?.decadeFortunes, calculation.luckCycles?.target?.year || currentAnnual?.year);
  const dominant = strongestElements(calculation).filter(Boolean).join('・') || '五行';
  const support = supportElements(calculation).filter(Boolean).join('・') || '調整五行';
  const headlineTags = readingTags.slice(0, 4);
  const overviewItems = [
    {
      label: '日主',
      value: `${calculation.dayMaster}（${calculation.pillars.day.element.stem}）`,
      note: STEM_READING[calculation.dayMaster]?.title || '本人の核',
      tone: 'core',
    },
    {
      label: '命式の型',
      value: displayPatternName(calculation.pattern),
      note: calculation.strength?.status || '身強弱',
      tone: 'pattern',
    },
    {
      label: '五行',
    ...
    {
      label: '現在の大運',
      value: currentDecade?.name || '—',
      note: currentDecade ? `${currentDecade.startAge}-${currentDecade.endAge}歳 / ${displayTenGod(currentDecade.pillar?.heavenlyTenGod)}` : '性別選択で定位',
      tone: 'trend',
    },
  ];
  return (
    <section id="s0" className="result-overview-panel result-wide" aria-label="鑑定結果の総覧">
      <div className="result-overview-head">
        <div>
          <div className="summary-kicker">四柱推命 鑑定結果</div>
          <h2>{name || 'あなた'}の命式</h2>
          <p>命盤の全体像を先に確認し、必要な項目へ進めます。</p>
        </div>
        <div className="result-overview-actions">
          <button type="button" onClick={onShowInsight}>命式詳細</button>
          <button type="button" onClick={onShowFortune}>大運・流年</button>
        </div>
      </div>
      <div className="result-overview-grid">
        {overviewItems.map(item => (
          <article key={item.label} className={`result-overview-card tag-${item.tone}`}>
      ...
      <div className="result-overview-tags" aria-label="要点">
        {headlineTags.map(tag => (
          <button key={`${tag.label}-${tag.value}-overview`} type="button" onClick={() => onShowInsight(tag.target || { topic: tag.topic })}>
            <span>{userTagIcon(tag)}</span>
            <b>{tag.label}</b>
            <em>{tag.value}</em>
          </button>
        ))}
...
function ResultView({ id, name, calculation, profile, onBack, onShowFortune, onShowInsight }) {
  ...
  const resultSections = [
    { id: 's0', num: '壹', label: '総覧' },
    { id: 's1', num: '貳', label: '命式表' },
    { id: 's2', num: '參', label: '要点解読' },
    { id: 's3', num: '肆', label: '次の読み' },
  ];
  const scrollTo = (sid) => {
  ...
  React.useEffect(() => {
    const onScroll = () => {
      const offset = window.matchMedia('(max-width: 768px)').matches ? 150 : 110;
      const current = resultSections
        .map(section => ({ ...section, el: document.getElementById(section.id) }))
        .filter(section => section.el)
        .reduce((active, section) => {
          const top = section.el.getBoundingClientRect().top - offset;
          return top <= 0 ? section.id : active;
        }, 's0');
  ...
  return (
    <section className="rite result-rite" data-screen-label="03 命式">
      <aside className="rite-side">
        ...
        <div className="seal-stack">
          {resultSections.map(section => (
            <button key={section.id} type="button" className={`side-topic ${activeSection === section.id ? 'is-active' : ''}`} onClick={() => scrollTo(section.id)}><span className="num">{section.num}</span><span>{section.label}</span></button>
          ))}
      ...
      <div className="rite-main result-main" style={{ paddingBottom: 120 }}>
        ...
        <nav className="result-mobile-nav" aria-label="結果ページ内メニュー">
          {resultSections.map(section => (
            <button key={section.id} type="button" className={activeSection === section.id ? 'is-active' : ''} onClick={() => scrollTo(section.id)}>{section.label}</button>
          ))}
        </nav>
        <div className="result-card" data-card-label="命式の確認" style={{ marginTop: 0 }}>
          <ResultOverviewPanel name={name} calculation={calculation} readingTags={readingTags} onShowInsight={onShowInsight} onShowFortune={onShowFortune} />
...
function DecadeFortuneMap({ decade, currentDecade }) {
  ...
  const chartPoints = items.map((item, index) => {
    ...
    const scoreRatio = clamp((item.score - 54) / 30, 0, 1);
    return {
      ...item,
      x: left + ratio * plotW,
      y: top + (1 - scoreRatio) * plotH,
    };
  });
  const linePath = chartPoints.map((point, index) => `${index === 0 ? 'M' : 'L'} ${point.x.toFixed(1)} ${point.y.toFixed(1)}`).join(' ');
  const areaPath = `${linePath} L ${chartPoints[chartPoints.length - 1].x.toFixed(1)} ${chartH - bottom} L ${chartPoints[0].x.toFixed(1)} ${chartH - bottom} Z`;
  const currentPoint = chartPoints.find(item => item.isCurrent) || chartPoints[0];
  return (
    <section className="fortune-map">
      ...
      <div className="fortune-curve" aria-label="十年ごとの運勢リズム">
        <svg viewBox={`0 0 ${chartW} ${chartH}`} role="img" aria-label="十年ごとの大運リズム">
          <defs>
            <linearGradient id="fortuneAreaGradient" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="rgba(201,42,36,.22)" />
              <stop offset="100%" stopColor="rgba(201,42,36,0)" />
            </linearGradient>
            <filter id="fortunePointShadow" x="-30%" y="-30%" width="160%" height="160%">
              <feDropShadow dx="0" dy="6" stdDeviation="5" floodColor="rgba(34,29,26,.18)" />
            </filter>
          </defs>
          {[0, .33, .66, 1].map((ratio, index) => {
            const y = top + ratio * plotH;
            return <line key={`grid-${index}`} className="fortune-grid-line" x1={left} x2={chartW - right} y1={y} y2={y} />;
          })}
          <line className="fortune-axis" x1={left} x2={chartW - right} y1={chartH - bottom} y2={chartH - bottom} />
          <text className="fortune-axis-label" x={left} y={top - 10}>動きが強い</text>
          <path className="fortune-area" d={areaPath} />
          <path className="fortune-line" d={linePath} />
          {chartPoints.map((item) => (
            <g key={item.index} className={item.isCurrent ? 'fortune-point is-current' : 'fortune-point'}>
              <line className="fortune-point-guide" x1={item.x} x2={item.x} y1={item.y + 14} y2={chartH - bottom} />
              <circle cx={item.x} cy={item.y} r={item.isCurrent ? 11 : 8} />
              <text className="fortune-age-label" x={item.x} y={chartH - 14}>{item.startAge}-{item.endAge}</text>
            </g>
          ))}
          {currentPoint && (
            <g className="fortune-current-badge" transform={`translate(${clamp(currentPoint.x - 74, left, chartW - right - 148).toFixed(1)} ${Math.max(10, currentPoint.y - 58).toFixed(1)})`}>
              <rect width="148" height="38" rx="19" />
              <text x="18" y="24">現在</text>
              <text x="62" y="24">{currentPoint.name} / {displayTenGod(currentPoint.pillar?.heavenlyTenGod)}</text>
            </g>
          )}
        </svg>
      </div>
    ...
    <section className="backend-panel luck-table-panel">
      ...
      {rows.length ? (
        <>
          ...
          <div className={`user-luck-card-list cols-${columns.length}`} aria-label={`${title}の明細`}>
            {rows.map((item, index) => {
              const primary = columns[1] || columns[0];
              const meta = columns[0];
              const detailColumns = columns.filter(col => col !== primary && col !== meta);
              return (
                <article key={`${title}-card-${index}`} className="user-luck-card">
                  <div className="user-luck-card-head">
                    <span>{meta.label} {meta.value(item)}</span>
                    <strong>{primary.value(item)}</strong>
                  </div>
                  <dl>
                    {detailColumns.map(col => (
                      <React.Fragment key={col.label}>
                        <dt>{col.label}</dt>
                        <dd>{col.value(item)}</dd>
                      </React.Fragment>
                    ))}
                  </dl>
                </article>
